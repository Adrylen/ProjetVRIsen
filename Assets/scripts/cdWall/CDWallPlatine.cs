...
public class CDWallPlatine : MonoBehaviour {
    ...
    void CreateCd(int y, int x, int i)
    {
        ...
        wall[y, x].GetComponent<CD>().parent = gameObject;
        wall[y, x].GetComponent<CD>().fileName = LoadResources.fileNames[i];
        wall[y, x].GetComponent<CD>().origin_position = new Vector3(0.3f * (3 - x), 0.27f * (3 - y) + 0.02f, 0.036f);
        wall[y, x].GetComponent<CD>().origin_scale = new Vector3(0.15f, 0.15f, 1);
